#!/bin/bash
# PrivateAI Quickstart - Mac 16GB RAM
# Ollama + dolphin3:8b + AnythingLLM
# privatedeskai.com | Version 1.3
# RUN: bash install-mac-dolphin3.sh
...
download_file() {
    ...
    # Wait up to 10 sec - antivirus or security software may briefly lock the file
    local av_wait=0
    while [ ! -f "$dest" ] && [ "$av_wait" -lt 10 ]; do
        sleep 2; av_wait=$(( av_wait + 2 ))
    done
    if [ ! -f "$dest" ]; then
        err "File missing after download: $dest. Security software may have removed it. Add exception for: $INSTALL_DIR"
    fi
    local final_mb
    final_mb=$(( $(stat -f%z "$dest") / 1024 / 1024 ))
    if [ "$final_mb" -lt 1 ]; then
        err "Downloaded file is too small (${final_mb} MB) - security software may have blocked it. Add exception for: $INSTALL_DIR"
    fi
...
echo "   Model: dolphin3:8b | Recommended: 16+ GB RAM"
...
if [ "$RAM_GB" -lt "$MODEL_RAM_MIN" ]; then
    warn "RAM: ${RAM_GB} GB. This model works best with ${MODEL_RAM_MIN}+ GB."
    warn "Performance may be slow. Consider using the 8GB version instead."
    echo ""
    read -r -p "Continue anyway? (y/n): " cont
    if [ "$cont" != "y" ] && [ "$cont" != "Y" ]; then exit 0; fi
else
    ok "RAM: ${RAM_GB} GB - sufficient for this model."
fi
...
if [ "$FREE_GB" -lt 10 ]; then
    err "Not enough disk space: ${FREE_GB} GB free. Need at least 10 GB."
fi
ok "Disk: ${FREE_GB} GB free - OK."

# Internet check
if ! curl -s --connect-timeout 10 https://ollama.com > /dev/null 2>&1; then
    err "No internet connection. Required for initial setup (~7 GB total download)."
fi
...
# ==============================================================
s "Step 3/5: Downloading AI model ($MODEL, ~4.9 GB)..."
echo "     This will take 10-30 minutes. Do not close this window."
echo ""
...
echo "   2. Click Manual setup"
echo "   3. Select Ollama as your LLM provider"
echo "   4. Choose model: dolphin3:8b"
echo "   5. Skip the email form, click arrow ->"
echo "   6. Click Open the assistant"
echo ""
echo "   IF WRONG MODEL IS SHOWN:"
echo "   Click the model name at the top of chat"
echo "   Select Ollama from provider list"
echo "   Choose dolphin3:8b"
echo ""
